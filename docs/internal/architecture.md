...
|---|---|---|
| v0.1 | Local, read-only inspection | in progress — five tools exist; `storage_health`, `system_info` and `system_health` run as probes, the two container tools are still in-process |
| v0.1 infrastructure | Workspace, core crate, embedded payload, probe chain on `local`, `xtask` release | in progress — workspace, `xtask dev`, a path-remapping `xtask release`, embedding, the probe chain on `local` and a shared probe runtime built (decisions 40–45); core now holds the types, the guard, the parsers and RFC 3339 (decision 35 carried out); no release matrix, manifest, extraction or disassembly check (decisions 33, 38, 39). 34 is reversed and is not part of the plan |
